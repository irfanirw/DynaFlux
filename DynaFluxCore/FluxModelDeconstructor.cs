...
{
    public static class FluxModelDeconstructor
    {
        public static void Deconstruct(FluxModel model, out string summary, out List<FluxSurface> surfaces)
        {
            if (model is null)
                throw new ArgumentNullException(nameof(model));

            surfaces = model.Surfaces?.ToList() ?? new List<FluxSurface>();
            ...
            sb.AppendLine($"Total FluxSurfaces: {surfaces.Count}");
