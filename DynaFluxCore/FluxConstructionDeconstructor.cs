...
namespace DynaFluxCore
{
    public static class FluxConstructionDeconstructor
    {
        ...
        public static string ToText(FluxConstruction construction)
        {
            if (construction == null) return "FluxConstruction: null";

            var sb = new StringBuilder();

            string type =
                construction is FluxFenestrationConstruction ? "Fenestration" :
                construction is FluxOpaqueConstruction ? "Opaque" :
                "Unknown";

            sb.AppendLine("FluxConstruction");
            ...
            if (construction.FluxMaterials != null && construction.FluxMaterials.Count > 0)
            {
                ...
                for (int i = 0; i < construction.FluxMaterials.Count; i++)
                {
                    var m = construction.FluxMaterials[i];
                    if (m == null) continue;
